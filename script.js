  ...
  tbody.innerHTML = suppliers.map((s) => {
    ...
    <tr>
      ...
      <td class="actions-cell">
        ...
        <button type="button" class="btn btn-sm btn-outline supplier-edit-btn" data-id="${escapeHtml(s.id)}" title="تعديل المورد">تعديل</button>
...
function openEditSupplierModal(supplierId) {
  ...
  document.getElementById('editSupplierOpeningBalance').value = supplier.openingBalance || 0;
  document.getElementById('editSupplierOpeningBalanceCurrency').value = supplier.openingBalanceCurrency || 'IQD';
...
document.getElementById('editSupplierModalClose')?.addEventListener('click', () => {
  document.getElementById('editSupplierModal').classList.remove('active');
});

document.getElementById('editSupplierCancelBtn')?.addEventListener('click', () => {
  document.getElementById('editSupplierModal').classList.remove('active');
});

document.getElementById('editSupplierModal')?.addEventListener('click', (e) => {
  if (e.target.id === 'editSupplierModal') document.getElementById('editSupplierModal').classList.remove('active');
});
...
document.getElementById('editSupplierForm')?.addEventListener('submit', (e) => {
  ...
  const phoneRaw = document.getElementById('editSupplierPhone').value.trim();
  const phone = phoneRaw.replace(/\D/g, '').slice(0, 11);
  const address = document.getElementById('editSupplierAddress').value.trim();
  const openingBalance = parseFloat(document.getElementById('editSupplierOpeningBalance').value) || 0;
  const openingBalanceCurrency = document.getElementById('editSupplierOpeningBalanceCurrency').value || 'IQD';
  if (!supplierId || !name || !phone || !address) return;
  const suppliers = getSuppliers();
  const supplier = suppliers.find((s) => s.id === supplierId);
  if (!supplier) return;
  supplier.name = name;
  ...
  supplier.openingBalanceCurrency = openingBalanceCurrency;
  setSuppliers(suppliers);
  addActivity('supplier', 'تعديل مورد: ' + name, phone);
  document.getElementById('editSupplierModal').classList.remove('active');
  renderSuppliersTable(document.getElementById('suppliersSearch')?.value);
  renderDashboard();
  fillSupplierSelect();
